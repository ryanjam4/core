...
def get_patient_data(request, patient_id):
    ...
    for problem in problems_query:
        ...
        d['goals'] = [{'id': g.id, 'start_date': g.start_date.strftime('%m/%d/%y'), 'goal': g.goal, 'is_controlled': g.is_controlled, 'accomplished': g.accomplished, 'notes': [{'note': n.note} for n in g.all().order_by('-datetime')]} for g in Goal.objects.filter(problem=problem, accomplished=False)]
