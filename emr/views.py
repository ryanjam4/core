...
def get_problems(request, user_id):
    ...
    for problem in Problem.objects.filter(patient=user_id):
        ...
        d['goals'] = [{'id': g.id, 'goal': g.goal, 'is_controlled': g.is_controlled, 'accomplished': g.accomplished, 'notes': {'by_physician': [{'note': n.note} for n in g.notes.objects.filter(by__in=['physician', 'admin']).order_by('-datetime')]}} for g in Goal.objects.filter(problem=problem, accomplished=False)]
